...
type SetIsChirpyRedUserParams struct {
	IsChirpyRed bool
	UserID      uuid.UUID
}

func (q *Queries) SetIsChirpyRedUser(ctx context.Context, arg SetIsChirpyRedUserParams) (User, error) {
	row := q.db.QueryRowContext(ctx, setIsChirpyRedUser, arg.IsChirpyRed, arg.UserID)
	var i User
	err := row.Scan(
		&i.ID,
		&i.CreatedAt,
